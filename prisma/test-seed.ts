...
async function main() {
  ...
  function generateResponse(templateId: string): { encrypted: string; iv: string; tag: string } {
    const questionPrefix = templateId === tplSWE ? "swe" : templateId === tplMgr ? "mgr" : templateId === tplPro ? "pro" : "simple";
    const answers: Record<string, number | string> = {};

    // Generate 5-10 rating answers with scores 2-5 (realistic range)
    const numRatings = 5 + Math.floor(Math.random() * 6);
    for (let i = 1; i <= numRatings; i++) {
      answers[`${questionPrefix}-q${i}`] = 2 + Math.floor(Math.random() * 4); // 2-5
    }
    // Add a couple text answers
    answers[`${questionPrefix}-text-strengths`] = "Great team player with strong technical skills. Consistently delivers high quality work.";
    answers[`${questionPrefix}-text-improve`] = "Could improve on documentation and cross-team communication.";
  ...
  console.log("  - Cross-team members (Alex in 2 teams, Sarah in 2 teams)");
