          ...
          {[1, 2].map((i) => (
            <Card key={i}>
              ...
              <div className="space-y-2">
                {[1, 2, 3].map((j) => <Skeleton key={j} className="h-14" />)}
              </div>
            </Card>
          ))}
        </div>
          ...
          <Card key={sIndex}>
            <CardHeader>
              <div className="flex items-center gap-2">
                <span className="flex items-center justify-center w-7 h-7 border border-gray-900 text-gray-900 text-[13px] font-semibold">{sIndex + 1}</span>
                <CardTitle>{section.title}</CardTitle>
              </div>
            </CardHeader>
            <div className="space-y-2">
              {section.questions.map((q, qIndex) => (
                <div key={qIndex} className="flex items-start justify-between p-3 bg-gray-50">
