...
export function SelfVsOthersChart({ data }: SelfVsOthersChartProps) {
  ...
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle>Self-Awareness Analysis</CardTitle>
      </CardHeader>
      ...
      <div className="px-1">
        <ResponsiveContainer width="100%" height={Math.max(200, data.length * 52 + 40)}>
          ...
          >
            ...
            />
            <ReferenceLine x={0} stroke="var(--gray-300)" />
            <Bar dataKey="Self" fill="#86868b" radius={[0, 4, 4, 0]} />
            <Bar dataKey="Others" fill="#0071e3" radius={[0, 4, 4, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    ...
    <div>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <div
            key={item.category}
            className="flex items-center justify-between text-[13px] py-1.5 px-3 rounded-lg bg-gray-50/60"
          >
