...
function DatePicker({
  ...
  return (
    <div className="space-y-2">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            disabled={disabled}
            className={cn(
              "flex w-full items-center h-11 px-3 border bg-white text-[15px] text-left",
              "focus:outline-none focus:outline-2 focus:outline-accent focus:outline-offset-2",
              "disabled:opacity-50 disabled:pointer-events-none",
              "border-gray-900",
              open && "outline outline-2 outline-accent outline-offset-2"
            )}
          >
            <CalendarIcon
              size={16}
      ...
      {error && <p className="text-[12px] text-gray-900">{error}</p>}
    </div>
  );
}
